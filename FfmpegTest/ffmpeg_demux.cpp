...
{
    const Demuxer* d = (const Demuxer*)obj;
    return d->log_name;
}
...
{
    const DemuxStream* ds = (const DemuxStream*)obj;
    return ds->log_name;
}
...
{
    Demuxer* d = (Demuxer*)allocate_array_elem((void*)(&input_files), sizeof(*d), &nb_input_files);
    if(!d)
        return NULL;
...
#define SHOW_TS_DEBUG(tag_)                                             \
    ...
    if(pkt->pts != AV_NOPTS_VALUE) {
        ...
        if(d->max_pts.ts == AV_NOPTS_VALUE ||
            ...
            d->max_pts = Timestamp(pkt->pts + pkt_duration, pkt->time_base );
        }
        if(d->min_pts.ts == AV_NOPTS_VALUE ||
            av_compare_ts(d->min_pts.ts, d->min_pts.tb,
                pkt->pts, pkt->time_base) > 0) {
            d->min_pts = Timestamp(pkt->pts, pkt->time_base);
        }
    }
...
static int input_packet_process(Demuxer* d, AVPacket* pkt, unsigned* send_flags)
{
    ...
    if(debug_ts) {
        AVRational temp = AV_TIME_BASE_Q;
        av_log(NULL, AV_LOG_INFO, "demuxer+ffmpeg -> ist_index:%d:%d type:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s duration:%s duration_time:%s off:%s off_time:%s\n",
            ...
            av_ts2str(pkt->dts), av_ts2timestr(pkt->dts, &pkt->time_base),
            av_ts2str(pkt->duration), av_ts2timestr(pkt->duration, &pkt->time_base),
            av_ts2str(f->ts_offset), av_ts2timestr(f->ts_offset, &temp));
    }
    ...
    while(1) {
        ...
        if(ret < 0) {
            ...
            if(d->loop) {
                ...
                if(ret >= 0)
                    ret = seek_to_start(d, Timestamp(dt.pkt_demux->pts, dt.pkt_demux->time_base));
                if(ret >= 0)
                    continue;
...
static DemuxStream* demux_stream_alloc(Demuxer* d, AVStream* st)
{
    ...
    ds = (DemuxStream*)allocate_array_elem(&f->streams, sizeof(*ds), &f->nb_streams);
    if(!ds)
        return NULL;
...
int ifile_open(const OptionsContext* o, const char* filename, Scheduler* sch)
{
    ...
    if(o->audio_ch_layouts.nb_opt)
    {
        ...
        if(file_iformat && (priv_class = file_iformat->priv_class) &&
            av_opt_find(&priv_class, "ch_layout", NULL, 0,
                AV_OPT_SEARCH_FAKE_OBJ))
        {
            av_dict_set(&o->g->format_opts, "ch_layout", (const char*)o->audio_ch_layouts.opt[o->audio_ch_layouts.nb_opt - 1].u.str, 0);
        }
    }
    if(o->frame_rates.nb_opt)
    {
                ...
                AV_OPT_SEARCH_FAKE_OBJ))
        {
            av_dict_set(&o->g->format_opts, "framerate",
                (const char*)o->frame_rates.opt[o->frame_rates.nb_opt - 1].u.str, 0);
        }
    }
    if(o->frame_sizes.nb_opt)
    {
        av_dict_set(&o->g->format_opts, "video_size", (const char*)o->frame_sizes.opt[o->frame_sizes.nb_opt - 1].u.str, 0);
    }
    if(o->frame_pix_fmts.nb_opt)
    {
        av_dict_set(&o->g->format_opts, "pixel_format", (const char*)o->frame_pix_fmts.opt[o->frame_pix_fmts.nb_opt - 1].u.str, 0);
    }
    ...
    d->duration = Timestamp(0, AVRational( 1, 1 ));
    d->min_pts = Timestamp(AV_NOPTS_VALUE, AVRational(1, 1));
    d->max_pts = Timestamp(AV_NOPTS_VALUE, AVRational(1, 1));
    ...
    for(i = 0; i < o->dump_attachment.nb_opt; i++)
    {
        for(int j = 0; j < f->nb_streams; j++)
        {
            ...
            {
                ret = dump_attachment(ist, (const char*)o->dump_attachment.opt[i].u.str);
                if(ret < 0)
                    return ret;
            }
        }
