...
std::atomic<unsigned> nb_output_dumped = 0;
...
static void uninit_options(OptionsContext* o)
{
    ...
    while(po->name)
    {
        ...
        if(po->flags & OPT_FLAG_SPEC) {
            SpecifierOptList* so = (SpecifierOptList*)dst;
        ...
        Scheduler*))
{
    ...
    for(int i = 0; i < l->nb_groups; i++)
    {
        ...
        // 解析输入输出文件选项参数并存储到OptionsContext
        int ret = parse_optgroup(&o, g, options);
        if(ret < 0) {
...
int ffmpeg_parse_options(int argc, char** argv, Scheduler* sch)
{
    ...
    /* split the commandline into an internal representation(分切命令行参数到octx对象中) */
    const char* errmsg = nullptr;
    int ret = split_commandline(&octx, argc, argv, options, groups, FF_ARRAY_ELEMS(groups));
    if(ret < 0)
    {
    ...
    /* apply global options (应用全局选项) 此处重新设置了一边loglevel为debug */
    ...
    char mediatype)
{
    ...
    for(int i = 0; i < sol->nb_opt; i++) {
        ...
        if(spec[0] == mediatype && !spec[1])
            return (const char*)sol->opt[i].u.str;
    }
    return NULL;
}
