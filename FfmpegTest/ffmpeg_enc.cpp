    ...
    uint64_t frame_num)
{
    ...
    std::unique_lock<std::mutex> lock(es->lock);
    ...
    for(size_t i = 0; i < es->nb_components; i++) {
        ...
        if(frame) {
            ...
            case ENC_STATS_KEYFRAME:    avio_write(io, (const unsigned char*)((pkt->flags & AV_PKT_FLAG_KEY) ?
                "K" : "N"), 1);                               continue;
...
static int update_video_stats(OutputStream* ost, const AVPacket* pkt, int write_vstats)
{
    ...
    pict_type = AVPictureType(sd ? sd[4] : AV_PICTURE_TYPE_NONE);
...
{
    OutputStream* ost = (OutputStream*)arg;
    Encoder* e = ost->enc;
    EncoderThread et;
...
static int set_encoder_id(OutputFile* of, OutputStream* ost)
{
    ...
    av_strlcat((char*)encoder_string, cname, encoder_string_len);
...
{
    OutputStream* ost = (OutputStream*)opaque;
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        enc_ctx->sample_fmt = (AVSampleFormat)frame->format;
    ...
    Scheduler* sch, unsigned sch_idx)
{
    ...
    *penc = NULL;
    ...
    enc = (Encoder*)av_mallocz(sizeof(*enc));
    if(!enc)
        return AVERROR(ENOMEM);
