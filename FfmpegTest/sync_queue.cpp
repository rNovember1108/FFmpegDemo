...
{
    const int planar = av_sample_fmt_is_planar((AVSampleFormat)f->format);
    const int planes = planar ? f->ch_layout.nb_channels : 1;
    const int    bps = av_get_bytes_per_sample((AVSampleFormat)f->format);
                           ...
                           AVFrame *dst, int nb_samples)
{
    ...
    while (dst->nb_samples < nb_samples) {
        ...
        av_samples_copy(dst->extended_data, src.f->extended_data, dst->nb_samples,
                        0, to_copy, dst->ch_layout.nb_channels, (AVSampleFormat)dst->format);
...
int sq_add_stream(SyncQueue *sq, int limiting)
{
    ...
    tmp = (SyncQueueStream*)av_realloc_array(sq->streams, sq->nb_streams + 1, sizeof(*sq->streams));
    if (!tmp)
...
{
    SyncQueue *sq = (SyncQueue*)av_mallocz(sizeof(*sq));
