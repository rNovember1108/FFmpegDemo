...
static int init_input_filter(FilterGraph* fg, InputFilter* ifilter)
{
    ...
    if(ifp->linklabel) {
        ...
        AVStream* st = NULL;
        char* p;
        int file_idx = strtol((const char*)ifp->linklabel, &p, 0);
    ...
    unsigned sched_idx_enc)
{
        ...
        if(ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
            ...
            if(!strcmp(c->name, "mjpeg")) {
                ...
                if(strict_val > FF_COMPLIANCE_UNOFFICIAL)
                    ofp->formats = (const int*)mjpeg_formats;
            }
        }
    ...
    case AVMEDIA_TYPE_AUDIO:
        if(ost->enc_ctx->sample_fmt != AV_SAMPLE_FMT_NONE) {
            ...
            ofp->formats = (const int*)c->sample_fmts;
        }
...
{
    const FilterGraphPriv* fgp = (const FilterGraphPriv*)obj;
    return fgp->log_name;
}
...
static InputFilter* ifilter_alloc(FilterGraph* fg)
{
    ...
    ifp = (InputFilterPriv*)allocate_array_elem(&fg->inputs, sizeof(*ifp), &fg->nb_inputs);
    if(!ifp)
        return NULL;
...
static OutputFilter* ofilter_alloc(FilterGraph* fg)
{
    ...
    ofp = (OutputFilterPriv*)allocate_array_elem(&fg->outputs, sizeof(*ofp), &fg->nb_outputs);
    if(!ofp)
        return NULL;
...
static int fg_thread_init(FilterGraphThread* fgt, const FilterGraph* fg)
{
    ...
    fgt->eof_in = (uint8_t*)av_calloc(fg->nb_inputs, sizeof(*fgt->eof_in));
    if(!fgt->eof_in)
        goto fail;

    fgt->eof_out = (uint8_t*)av_calloc(fg->nb_outputs, sizeof(*fgt->eof_out));
    if(!fgt->eof_out)
        goto fail;
    ...
    InputFilter* ifilter, AVFilterInOut* in)
{
    ...
    desc = av_pix_fmt_desc_get((AVPixelFormat)ifp->format);
    av_assert0(desc);
    ...
    InputFilter* ifilter, AVFilterInOut* in)
{
    ...
    av_bprintf(&args, "time_base=%d/%d:sample_rate=%d:sample_fmt=%s",
        ...
        ifp->sample_rate,
        av_get_sample_fmt_name((AVSampleFormat)ifp->format));
    ...
    *dst = NULL;
    ...
    if(ost->keep_pix_fmt || ofp->format != AV_PIX_FMT_NONE) {
        *dst = ofp->format == AV_PIX_FMT_NONE ? NULL :
            av_get_pix_fmt_name((AVPixelFormat)ofp->format);
    }
    else if(ofp->formats) {
...
static void choose_sample_fmts(OutputFilterPriv* ofp, AVBPrint* bprint)
{
    if(ofp->format == AV_SAMPLE_FMT_NONE && !ofp->formats)
        return;
    av_bprintf(bprint, "sample_fmts" "=");
    if(ofp->format != AV_SAMPLE_FMT_NONE) {
        av_bprintf(bprint, "%s", av_get_sample_fmt_name((AVSampleFormat)ofp->format));
    }
    else
    {
        const enum AVSampleFormat* p;
        for(p = (AVSampleFormat*)ofp->formats;
            *p != AV_SAMPLE_FMT_NONE; p++)
        {
            av_bprintf(bprint, "%s" "|", av_get_sample_fmt_name(*p));
        }
        if(bprint->len > 0)
            bprint->str[--bprint->len] = '\0';
    }
    ...
    if(need_reinit || !fgt->graph) {
        ...
        if(fgt->graph) {
            ...
            if(need_reinit & AUDIO_CHANGED) {
                const char* sample_format_name = av_get_sample_fmt_name((AVSampleFormat)frame->format);
            ...
            if(need_reinit & VIDEO_CHANGED) {
                const char* pixel_format_name = av_get_pix_fmt_name((AVPixelFormat)frame->format);
...
{
    FilterGraphPriv* fgp = (FilterGraphPriv*)arg;
    ...
    while(1) {
        ...
        o = (FrameOpaque)((intptr_t)fgt.frame->opaque);
...
int fg_create(FilterGraph** pfg, char* graph_desc, Scheduler* sch)
{
    ...
    fgp = (FilterGraphPriv*)allocate_array_elem(&filtergraphs, sizeof(*fgp), &nb_filtergraphs);
    if(!fgp)
        return AVERROR(ENOMEM);
    fg = &fgp->fg;
    ...
    const char* command, const char* arg, int all_filters)
{
    ...
    fc = (FilterCommand*)av_mallocz(sizeof(*fc));
    if(!fc)
        return;
